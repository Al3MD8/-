...
window.createAnimeCardHTML = function(anime) {
  ...
  const statusText = anime.status === 'Currently Airing' ? '┘è╪╣╪▒╪╢ ╪º┘ä╪º┘å' : (anime.status === 'Not yet aired' ? '┘é╪▒┘è╪¿╪º┘ï' : '┘à┘â╪¬┘à┘ä');
      ...
      <div class="card-img-wrapper" onclick="openAnimeDetails(${anime.mal_id})">
        ...
        <div class="card-overlays">
          <div class="card-top-badges">
            ...
            <button class="card-save-btn ${saveBtnSavedClass}" data-id="${anime.mal_id}" aria-label="╪¡┘ü╪╕ ╪º┘ä╪ú┘å┘à┘è" onclick="event.stopPropagation(); toggleBookmark(window.getAnimeDataObj(${anime.mal_id}))">
...
window.renderBrowsePage = async function() {
  ...
  if(items.length === 0) {
    grid.innerHTML = '<div class="empty-state"><h3>┘ä╪º ╪¬┘ê╪¼╪» ┘å╪¬╪º╪ª╪¼ ┘à╪╖╪º╪¿┘é╪⌐ ┘ä╪¬╪╡┘ü╪¡┘â</h3></div>';
  } else {
...
function renderSavedAnimeSPA() {
  ...
  if(state.savedAnimesData.length === 0) {
    grid.innerHTML = '<div class="empty-state"><h3>┘é╪º╪ª┘à╪¬┘â ╪º┘ä┘à╪¡┘ü┘ê╪╕╪⌐ ┘ü╪º╪▒╪║╪⌐ ╪¡╪º┘ä┘è╪º┘ï.</h3></div>';
  } else {
...
window.renderWatchHistoryGridSPA = function() {
  ...
  if(history.length === 0) {
    grid.innerHTML = '<div class="empty-state"><h3>╪│╪¼┘ä ╪º┘ä┘à╪┤╪º┘ç╪»╪⌐ ╪º┘ä╪«╪º╪╡ ╪¿┘â ┘ü╪º╪▒╪║</h3></div>';
    return;
  }
  ...
  grid.innerHTML = history.map(item => {
    ...
    const title = item.title || item.anime?.title || '╪ú┘å┘à┘è';
    ...
    return `
      <article class="anime-card" style="cursor:pointer;" onclick="openLatestEpisode(${id}, ${item.episodeIndex})">
        <div class="card-img-wrapper" onclick="openLatestEpisode(${id}, ${item.episodeIndex})">
          ...
          <div class="card-overlays">
            <span class="card-badge" style="background:#ff69b4;">╪¡ ${epNum}</span>
          </div>
        </div>
...
window.openAnimeDetails = async function(animeId) {
  ...
  document.getElementById('detailsDesc').textContent = anime.synopsis || '┘ä╪º ┘è┘ê╪¼╪» ┘ê╪╡┘ü ┘à╪¬╪º╪¡.';
  ...
  document.getElementById('detailsStatus').textContent = anime.status === 'Currently Airing' ? '┘à╪│╪¬┘à╪▒' : '┘à┘â╪¬┘à┘ä';
  ...
  const isSaved = state.savedAnimeIds.includes(anime.mal_id);
  saveBtn.innerHTML = isSaved ? '<i class="fa-solid fa-bookmark"></i> ┘à╪¡┘ü┘ê╪╕' : '<i class="fa-regular fa-bookmark"></i> ╪¡┘ü╪╕';
  ...
  const relationsContainer = document.getElementById('detailsRelationsSection') || (() => {
    ...
    section.innerHTML = `
      <h2 style="font-size:1.3rem;color:var(--accent-color);margin-bottom:20px;"><i class="fa-solid fa-layer-group"></i> ┘à┘ê╪º╪│┘à ┘ê╪ú╪¼╪▓╪º╪í ╪░╪º╪¬ ╪╡┘ä╪⌐</h2>
  ...
  if(episodes.length === 0 && total > 0) episodes = Array.from({length: total}, (_,i) => ({mal_id:i+1, title:'╪º┘ä╪¡┘ä┘é╪⌐ '+(i+1)}));
  if(episodes.length === 0) episodes = Array.from({length: 12}, (_,i) => ({mal_id:i+1, title:'╪º┘ä╪¡┘ä┘é╪⌐ '+(i+1)}));
  ...
  grid.innerHTML = episodes.map((ep,idx) => `
    <button class="ep-card-btn" onclick="openPlayerView(${idx})">
      <i class="fa-solid fa-play-circle"></i> ╪º┘ä╪¡┘ä┘é╪⌐ ${idx+1}
    </button>
  `).join('');
  ...
  try {
    ...
    if(relatedAnime.length > 0) {
      ...
      relGrid.innerHTML = detailedRels.map(rel => {
        ...
        let badgeText = rel.relation === 'Sequel' ? '╪¬┘â┘à┘ä╪⌐ (┘à┘ê╪│┘à ╪¬╪º┘ä┘è)' : rel.relation === 'Prequel' ? '┘à┘ê╪│┘à ╪│╪º╪¿┘é' : '┘é╪╡╪⌐ ╪░╪º╪¬ ╪╡┘ä╪⌐';
      ...
      relGrid.innerHTML = '<p style="color:var(--text-sub);">┘ä╪º ╪¬┘ê╪¼╪» ┘à┘ê╪º╪│┘à ╪ú╪«╪▒┘ë ┘à╪╣╪▒┘ê┘ü╪⌐ ┘ä┘ç╪░╪º ╪º┘ä╪ú┘å╪¿┘è.</p>';
    }
  } catch(e) {
    relGrid.innerHTML = '<p style="color:var(--text-sub);">╪¬╪╣╪░╪▒ ╪¼┘ä╪¿ ╪º┘ä┘à┘ê╪º╪│┘à.</p>';
  }
};
...
function classifyServerQuality(name) {
  ...
  if(n.includes('fhd') || n.includes('1080') || n.includes('drive') || n.includes('google') || n.includes('mega') || n.includes('fembed') || n.includes('okru') || n.includes('ok.ru') || n.includes('vidlink (┘à╪¬╪▒╪¼┘à)')) {
    return 'fhd';
  }
  return 'hd';
}
    ...
    return `
      <article class="anime-card history-card" data-id="${id}" onclick="window.resumeWatchHistory(${id}, ${item.episodeIndex})">
        <div class="card-img-wrapper">
          <img src="${poster}" alt="${title}" loading="lazy" onerror="this.src='https://via.placeholder.com/300x400?text=No+Image'">
          <div class="card-overlays">
            <span class="card-badge" style="background: #ff69b4; box-shadow: 0 0 10px rgba(255, 105, 180, 0.6); pointer-events: none;"><i class="fa-solid fa-clock"></i> ╪¡ ${epNum}</span>
          </div>
        ...
        <div class="card-info" style="padding: 12px;">
          ...
          <div class="card-meta-info" style="display:flex; justify-content:space-between; align-items:center; font-size:0.75rem; color:var(--text-sub);">
            <span style="font-size: 0.72rem; color: #ff69b4; font-weight: 700; cursor: pointer;"><i class="fa-solid fa-circle-play"></i> ╪º╪│╪¬╪ª┘å╪º┘ü ╪¡ ${epNum}</span>
...
function showHeroSlide(index) {
  ...
  setTimeout(() => {
    ...
    document.getElementById('heroEpisodes').textContent = `${featured.episodes || '?'} ╪¡┘ä┘é╪⌐`;
...
function updateHeroBookmarkStateForSlide(malId) {
  ...
  if (isSaved) {
    btn.innerHTML = `<i class="fa-solid fa-bookmark"></i> ┘à╪¡┘ü┘ê╪╕`;
    btn.classList.add('saved');
  } else {
    btn.innerHTML = `<i class="fa-regular fa-bookmark"></i> ╪¡┘ü╪╕`;
...
window.showAuthTab = function(tab) {
  ...
  document.getElementById('authSubmitBtn').innerHTML = tab==='login' ? '<i class="fa-solid fa-right-to-bracket"></i> ╪¬╪│╪¼┘è┘ä ╪º┘ä╪»╪«┘ê┘ä' : '<i class="fa-solid fa-user-plus"></i> ╪Ñ┘å╪┤╪º╪í ╪¡╪│╪º╪¿';
...
window.openAuthModal = function(forceEditMode = false) {
  ...
  if(userState.isLoggedIn) {
     ...
     document.getElementById('authModalTitle').innerHTML = '<i class="fa-solid fa-user-check highlight"></i> ┘à┘ä┘ü┘â ╪º┘ä╪┤╪«╪╡┘è';
  } else {
     ...
     document.getElementById('authModalTitle').innerHTML = '<i class="fa-solid fa-user-gear highlight"></i> ╪¡╪│╪º╪¿┘è';
     showAuthTab('login');
  }
};
...
async function handleAuth() {
  ...
  if(!user || !pass) { errEl.textContent = '┘è╪▒╪¼┘ë ┘à┘ä╪í ╪¼┘à┘è╪╣ ╪º┘ä╪¡┘é┘ê┘ä'; errEl.style.display = 'block'; return; }
  ...
  } catch(e) { errEl.textContent = '╪«╪╖╪ú ┘ü┘è ╪º┘ä╪º╪¬╪╡╪º┘ä ╪¿╪º┘ä╪│┘è╪▒┘ü╪▒'; errEl.style.display = 'block'; }
}
...
async function loadComments() {
  ...
  } catch(e) { container.innerHTML = '<p style="color:var(--text-sub);text-align:center;">╪¬╪╣╪░╪▒ ╪¬╪¡┘à┘è┘ä ╪º┘ä╪¬╪╣┘ä┘è┘é╪º╪¬</p>'; }
}
...
window.setReplyTo = function(commentId, username) {
  ...
  const input = document.getElementById('commentInput');
  if(input) {
    input.placeholder = `╪º┘â╪¬╪¿ ╪▒╪»┘â ╪╣┘ä┘ë @${username}...`;
    input.focus();
  }
};
...
window.cancelReply = function() {
  ...
  const input = document.getElementById('commentInput');
  if(input) input.placeholder = "╪º┘â╪¬╪¿ ╪¬╪╣┘ä┘è┘é┘â ┘ç┘å╪º... (╪ú┘ê ╪º┘ä╪╡┘é ╪╡┘ê╪▒╪⌐ ┘à╪¿╪º╪┤╪▒╪⌐!)";
};

window.deleteComment = async function(commentId) {
  if(!confirm("┘ç┘ä ╪ú┘å╪¬ ┘à╪¬╪ú┘â╪» ┘à┘å ╪¡╪░┘ü ╪¬╪╣┘ä┘è┘é┘â ┘å┘ç╪º╪ª┘è╪º┘ï╪ƒ")) return;
  const key = getEpisodeKey();
  try {
...
function renderComments(comments) {
  const container = document.getElementById('commentsContainer');
  if(!container) return;
  if(comments.length === 0) { container.innerHTML = '<p style="color:var(--text-sub);text-align:center;padding:20px;">┘ä╪º ╪¬┘ê╪¼╪» ╪¬╪╣┘ä┘è┘é╪º╪¬ ╪¿╪╣╪». ┘â┘å ╪ú┘ê┘ä ┘à┘å ┘è╪╣┘ä┘é!</p>'; return; }
      ...
      <div style="display:flex; gap:12px;">
        ...
        <div style="flex:1; min-width:0;">
          ...
          <div style="display:flex; gap:15px; align-items:center;">
            ...
            ${isOwner ? `<button onclick="deleteComment('${c.id}')" style="background:none; border:none; color:#ff4757; cursor:pointer; font-family:inherit; font-size:0.85rem; display:flex; align-items:center; gap:4px;"><i class="fa-solid fa-trash"></i> ╪¡╪░┘ü</button>` : ''}
          </div>
        </div>
      </div>
      ${repliesHtml}
...
window.renderLatestEpisodes = async function() {
  ...
  try {
    ...
    try {
      ...
      if (res.ok) {
        ...
        watchItems.forEach(item => {
          ...
          let epStr = item.episodes && item.episodes.length > 0 ? item.episodes[0].title : "╪º┘ä╪¡┘ä┘é╪⌐ 1";
    ...
    if (seasonal.length === 0) {
      grid.innerHTML = '<p style="color:var(--text-sub); text-align:center;">┘ä╪º ╪¬┘ê╪¼╪» ╪¡┘ä┘é╪º╪¬ ┘à╪¬╪º╪¡╪⌐ ╪¡╪º┘ä┘è╪º┘ï.</p>';
      return;
    }
    ...
    const processedEpisodes = seasonal.map(anime => {
      ...
      if (anime.broadcast && anime.broadcast.day) {
        const daysMap = {
          'Mondays': '╪º┘ä╪Ñ╪½┘å┘è┘å', 'Tuesdays': '╪º┘ä╪½┘ä╪º╪½╪º╪í', 'Wednesdays': '╪º┘ä╪ú╪▒╪¿╪╣╪º╪í',
          'Thursdays': '╪º┘ä╪«┘à┘è╪│', 'Fridays': '╪º┘ä╪¼┘à╪╣╪⌐', 'Saturdays': '╪º┘ä╪│╪¿╪¬', 'Sundays': '╪º┘ä╪ú╪¡╪»'
        };
      ...
      const statusText = anime.status === 'Currently Airing' ? '┘à╪│╪¬┘à╪▒' : '┘à┘â╪¬┘à┘ä';
    ...
    grid.innerHTML = processedEpisodes.slice(0, 8).map(item => {
      ...
      const releaseText = item.dayArabic ? `╪¡┘ä┘é╪⌐ ╪¼╪»┘è╪»╪⌐ ┘â┘ä ${item.dayArabic}` : '╪ú╪╢┘è┘ü ┘ç╪░╪º ╪º┘ä╪ú╪│╪¿┘ê╪╣';
      const poster = anime.images?.webp?.large_image_url || anime.images?.jpg?.large_image_url || 'https://via.placeholder.com/300x400?text=No+Image';
      
      const badgeColor = item.statusText === '┘à╪│╪¬┘à╪▒' ? '#00FF66' : '#a8a8a8';
      const borderGlow = item.statusText === '┘à╪│╪¬┘à╪▒' ? 'rgba(0, 255, 102, 0.4)' : 'rgba(168, 168, 168, 0.4)';
      ...
      return `
        <article class="anime-card episode-card" data-id="${anime.mal_id}">
          <div class="card-img-wrapper" onclick="window.openLatestEpisode(${anime.mal_id}, ${epNum - 1})">
            <img src="${poster}" alt="${anime.title}" loading="lazy" onerror="this.src='https://via.placeholder.com/300x400?text=No+Image'">
            <div class="card-overlays">
              <span class="card-badge" style="background: #00a8cc; box-shadow: 0 0 10px rgba(0, 168, 204, 0.6); pointer-events: none;"><i class="fa-solid fa-play"></i> ╪º┘ä╪¡┘ä┘é╪⌐ ${epNum}</span>
          ...
          <div class="card-info" style="padding: 12px;">
            ...
            <div class="card-meta-info" style="display:flex; justify-content:space-between; align-items:center; font-size:0.75rem; color:var(--text-sub);">
              ...
              <span style="font-size: 0.72rem; color: #888;">┘à┘å╪░ ╪ú┘è╪º┘à</span>
            </div>
          </div>
        </article>
      `;
    ...
    grid.innerHTML = '<p style="color:var(--text-sub); text-align:center;">╪¬╪╣╪░╪▒ ╪¬╪¡┘à┘è┘ä ╪º┘ä╪¡┘ä┘é╪º╪¬</p>';
  }
};
